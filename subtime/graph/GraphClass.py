...
from subtime.graph.Edge import Edge
from subtime.graph.Endpoint import Endpoint
from subtime.graph.GeneralGraph import GeneralGraph
from subtime.graph.GraphNode import GraphNode
from subtime.graph.Node import Node
from subtime.utils.GraphUtils import GraphUtils
from subtime.utils.PCUtils.Helper import list_union, powerset
...
class CausalGraph:
    def __init__(self, no_of_var: int, node_names: List[str] | None = None):
        ...
        self.PC_elapsed = -1  # store the elapsed subtime of running PC
    ...
    def draw_nx_graph(self, skel=False,filename=None,figsize=(30,20)):
        ...
        # custom, added to avoid overlapped plots
    ...
    # custom
    def save_dig(self, filename):
        r"""
        Functs: - to_nx_graph if self.nx_graph is None
                - save the graph to gml file under filename
        """
        if len(self.nx_graph.edges) == 0:
            self.to_nx_graph()

        nx.write_gml(self.nx_graph, filename)
