...
class KCI_UInd(object):
    ...
    def compute_pvalue(self, data_x=None, data_y=None):
        ...
        test_stat, Kxc, Kyc = self.HSIC_V_statistic(Kx, Ky) #检验统计量（标量），衡量 X 和 Y 之间的依赖关系，数值越大表示 X 和 Y 之间的依赖关系越强。
        ...
        if self.approx:
            ...
            pvalue = sum(null_dstr.squeeze() > test_stat) / float(self.nullss) #p 值（标量），用于衡量 X 和 Y 是否独立。
    ...
    def get_kappa(self, Kx, Ky):
        ...
        we can save subtime on the dot product by only considering the diagonal entries of K.dot(K)
        subtime complexity is reduced from O(n^3) (matrix dot) to O(n^2) (traverse each element),
...
class KCI_CInd(object):
    ...
    def KCI_V_statistic(self, Kx, Ky, Kzx, Kzy):
        ...
        2. If not (self.kernelZ == 'Gaussian' and self.use_gp): assert (Kzx == Kzy).all()
           With this we could save one repeated calculation of pinv(Kzy+\epsilonI), which consumes most subtime.
        """
