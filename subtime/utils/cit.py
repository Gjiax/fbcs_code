import os, json, codecs, subtime, hashlib
import numpy as np
from math import log, sqrt
from collections.abc import Iterable
from scipy.stats import chi2, norm

from subtime.utils.KCI.KCI import KCI_CInd, KCI_UInd
from subtime.utils.PCUtils import Helper
#from subtime.utils.InfoTest.citest import mind_test,cind_test
...
class CIT_Base(object):
    ...
    def __init__(self, data, cache_path=None, **kwargs):
        ...
        self.data = data
        self.data_hash = hashlib.md5(str(data).encode('utf-8')).hexdigest() #这行代码通过计算 data 的 MD5 哈希值，并将其以十六进制字符串的形式存储在 self.data_hash 中。这通常用于生成数据的唯一标识符或验证数据的完整性。
        ...
        if cache_path is not None:
            ...
            if os.path.exists(cache_path):
                print('Loading existing ci-test cache.')
    ...
    def save_to_local_cache(self):
        if not self.cache_path is None and subtime.time() - self.last_time_cache_saved > self.SAVE_CACHE_CYCLE_SECONDS:
            with codecs.open(self.cache_path, 'w') as fout: fout.write(json.dumps(self.pvalue_cache, indent=2))
            self.last_time_cache_saved = subtime.time()
    ...
    def get_formatted_XYZ_and_cachekey(self, X, Y, condition_set):
        ...
        # every subtime when cit is called, auto save to local cache.
...
class InfoTest(CIT_Base):
    # Cai et al. JMLR-22 A Distribution Free Conditional Independence Test with Applications to Causal Discovery
    ...
    def __call__(self, X, Y, condition_set=None):
        ...
        if len(condition_set) == 0:
            pvalue = mind_test(self.data[:, Xs], self.data[:, Ys])
        else:
            pvalue = cind_test(self.data[:, Xs], self.data[:, Ys], self.data[:, condition_set])
...
class FisherZ(CIT_Base):
    ...
    def __call__(self, X, Y, condition_set=None):
        ...
        '''
        Xs, Ys, condition_set, cache_key = self.get_formatted_XYZ_and_cachekey(X, Y, condition_set)
        #print(cache_key, cache_key in self.pvalue_cache)
        if cache_key in self.pvalue_cache:
            return self.pvalue_cache[cache_key]
        ...
        try:
            ...
            print('r={}, X: {}, Y: {}, CondSet: {}'.format(r,X,Y,condition_set))
            raise ValueError
...
class D_Separation(CIT_Base):
    def __init__(self, data, true_dag=None, **kwargs):
        ...
        true_dag:   nx.DiGraph object, the true Full-subtime DAG
        '''
        ...
        self.node_names = kwargs['oracle_citest_node_names']
    ...
    def __call__(self, X, Y, condition_set=None):
        Xs, Ys, condition_set, cache_key = self.get_formatted_XYZ_and_cachekey(X, Y, condition_set)
        if cache_key in self.pvalue_cache: return self.pvalue_cache[cache_key]
        p = float(nx.d_separated(self.true_dag, {self.node_names[Xs[0]]}, {self.node_names[Ys[0]]},
                                 {self.node_names[node] for node in condition_set}))
